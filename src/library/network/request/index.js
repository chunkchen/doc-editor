...
export const get = (url, parameters, options) => {
  const realPath = parameters ? ''.concat(url, '?')
    .concat(queryString.stringify(parameters)) : url;
  return doFetch(realPath, options);
};
...
export const namespaceDelete = (url, parameters, options) => {
  const realPath = parameters && Object.keys(parameters).length ? ''.concat(url, '?')
    .concat(queryString.stringify(parameters)) : url;
  return doFetch(realPath, {
    method: 'DELETE',
    ...options,
  });
};
