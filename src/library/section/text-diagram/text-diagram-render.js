...
class TextDiagramViewer extends React.Component {
  ...
  genImg() {
    preview(this.props.type, this.props.code)
      .then((res) => {
        if (!res.success) {
          this.setState({
            error: res.message,
          ...
          const src = `data:image/svg+xml,${encodeURIComponent(res.svg)
            .replace(/'/g, '%27')
            .replace(/"/g, '%22')}`
          this.setState({
            url: src,
  ...
  render() {
    ...
    return (
      <div
        className={'lake-text-diagram-viewer lake-text-diagram-'.concat(type, ' ')
          .concat(className)}
      >
        {
          children
