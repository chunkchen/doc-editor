...
class TocSidebar extends React.Component {
  constructor(props) {
    ...
    this.handleClick = (e, id) => {
      ...
      const currentItem = Engine.$(e.target)
        .closest('.lake-toc-item')
      ...
      root.find('.lake-toc-item-active')
        .removeClass('lake-toc-item-active')
      itemList.eq(itemIndex)
        .addClass('lake-toc-item-active')
      const block = engine.container.find('[id="'.concat(id, '"]'))
        .eq(itemIndex)
      const contentArea = engine.container.closest('.lake-max-editor-wrapper-content')
      const toolbarArea = engine.container.closest('.lake-max-editor')
        .find('.lake-toolbar')
      // 需要减掉工具栏和上面元素的高度
